...
- **`styles.css`** — Tailwind v4 config (`@theme` variables, `@utility`, `@layer base/components`), dark mode via
  `html.dark` class, tab group color system
...
2. **CSS** — `@apply` rules in `styles.css` for dynamically-created elements (e.g. `details`, `summary`, `ul`, `li`) and
   custom element selectors (e.g. `bookmark-link`, `tab-link`)
3. **JS** — `className` / `classList` in TypeScript as a last resort, only when styles are truly dynamic (e.g.
   `overflow-y-auto` on layout columns, `gridColumn` for responsive layout)
